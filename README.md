...
- 默认所有数据（进度、心得、激励）仅保存在**本机浏览器 localStorage**（键名 `zhinang-jingdu-v1`），不上传任何服务器
- **多设备同步**有两种方式：
  - **导出 / 导入**：阶段总结页导出 JSON，经微信文件传输助手等发到另一台设备导入；导入为「逐则合并」——同一则取较新版本，本机记录不会被旧数据覆盖
  - **家庭云同步**：在「阶段总结 → ☁️ 家庭云同步」粘贴一个 Gitee 私人令牌（设置 → 私人令牌 → 勾选 projects 权限生成），数据即保存在你自己账号的私有仓库中；多台设备粘贴同一令牌即共享进度，每次保存自动上传、打开自动拉取合并。不开启则完全离线可用
- 换设备或清理浏览器前请先导出 JSON 备份；清除浏览器数据会清空学习记录
